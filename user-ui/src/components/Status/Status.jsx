...
const Status = () => {
  const { connectWallet, changeStatus, message, isLoading } =
    ...
    <>
      ...
      <div className="bg-white">
        <p className="text-center text-gray-400 mb-16">
          don't forget to connect to your wallet
        </p>
      </div>

      <div>
        <a
          className="arrow mb-4 bg-gradient-to-r from-black via-gray-300"
          onClick={routeChange}
        >
          Back
        </a>
      ...
      <div
        data-testid="show"
        className="container contain-1 text-gray-600 font-serif flex "
      >
        <form className="form-status status-form text-black shadow-2xl px-5 mx-20 my-20 bg-white flex">
          <div className="textbox my-36 text-2xl">
            <p className="mb-5 text-2xl">Change users' Nft status</p>
            <div className="text-xl text-gray-500">{message}</div>
          </div>
          <div className="fields px-10">
            <div className="mb-10 py-3 text-xl text-black">
              <h1 className="tit">Change IP Status</h1>
            </div>
            <div className="">
              <div className="mb-6">
                <label className="text-lg">Nft token ID </label>
                <br></br>
                <input
                  className="input-box text-gray-700 border py-2 px-2 rounded"
                  type="text"
                  name="id"
                  ref={id}
                  value={item.tokenId}
                  placeholder="id number of the intellectual property"
                />
              </div>
              <div className="mb-4">
                <label className="text-lg">Nft Status</label>
                <br></br>
              ...
              <div className="py-3">
                <button
                  ...
                  className="mt-5 w-28 py-3 bg-black rounded text-white text-lg cursor-pointer"
                >
                  Change
                </button>
              </div>
              {isLoading ? (
                <p className="text-red-600 text-sm">loading...</p>
              ) : null}
            </div>
          </div>
        </form>
